...
@greyscale.command()
@click.pass_context
def interpolate(ctx: click.Context, average: bool, fixture: bool, interactive: bool, overwrite: bool) -> None:
    ...
    if not compilation_has_missing_greyscales(compilation):
    ...
    if interactive and backup.exists() and overwrite is None:
        overwrite = Confirm.ask(f"Found a compilation backup at '{backup}'. Do you want to overwrite it?", default=False)
    ...
    if (interactive or average):
        ...
        if greyscales is None:
            ...
            if stdev > 1:
                LOGGER.warning("High standard deviation (%.2f) in greyscale values from compilation '%s'.", stdev, compilation.name)
    else:
        greyscales = None
    ...
    if interactive:
        ...
        response = Prompt.ask(
            ...
            default="average" if (average is not None and stdev <= 1) else "fixture",
        )
    ...
    try:
        replace_missing_greyscales_in_compilation(compilation, backup, greyscale)
